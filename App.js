...
export default function App() {
	...
	const GoogleAPIKey = "AIzaSyCaIbTP4TCTCaCVfu6VOq2HJmFoHIsK9a8";
	...
	const getTranslation = async (className) => {
		try {
			...
			if (!apiCall) {
				console.error(
					`Google API did not respond adequately. Review API call.`
				);
		...
		return (
			<View style={styles.cameraView}>
				...
				<Text style={styles.legendTextField}>
					Point to any object and get its{" "}
					{availableLanguages.find((al) => al.value === language).label}{" "}
					translation
				</Text>
			</View>
		);
